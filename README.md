...
# Figures 재생성
conda run -n mett --no-capture-output python python/generate_figures.py
```

## R 원본 패키지 배치 (검증·비교용)

R 원본 `METT` 패키지는 라이선스/배포 정책에 따라 본 레포에는 포함되지 않습니다.
검증(rpy2 oracle) 및 벤치마크를 재현하려면 Park 교수님 사이트에서 직접 받아
**아래 디렉토리 구조에 그대로 배치**하세요.

- 배포 페이지: <https://sites.google.com/view/yeonheepark/research> (Google Drive 링크)
- 받은 파일을 풀어 다음 위치에 둡니다 (.gitignore 처리되어 커밋되지 않습니다):
  - `R/METT_update.R`
  - `man/METT-package.Rd`, `man/METT2E.Rd`, `man/METT2U.Rd`, `man/METT2W.Rd`
...
├── R/METT_update.R          # ★ Park 사이트에서 받아 배치 (gitignored)
├── man/                     # ★ Park 사이트에서 받아 배치 (gitignored)
├── DESCRIPTION              # ★ Park 사이트에서 받아 배치 (gitignored)
├── NAMESPACE                # ★ Park 사이트에서 받아 배치 (gitignored)
├── python/                  # PyTorch 포팅 (이번 작업)
│   ├── 01_validation.ipynb       # 라이브 검증 노트북
...
★ 표시 항목은 Park 교수님 사이트에서 받아 로컬에 배치해야 하는 R 원본 파일입니다.
...
상세 검증 절차·결과·설계 결정은 [`python/01_validation.ipynb`](python/01_validation.ipynb) 의 검증 노트북 참조.
